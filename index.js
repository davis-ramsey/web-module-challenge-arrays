...
// 	const newArray = [];
// 	for (let i = 0; i < arr.length; i++) if (arr[i].includes(string)) newArray.push(arr[i]);
// 	return newArray;
// }
...
// 	const allFlavors = [];
// 	for (let array of flavors) for (let item of array) allFlavors.push(item);
// 	const flavorList = [];
// 	for (let i = 0; i < 31; i++) {
// 		const index = Math.floor(Math.random() * allFlavors.length);
// 		flavorList.push(allFlavors[index]);
